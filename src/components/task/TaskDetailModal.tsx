...
export interface TaskDetailModalProps {
  open: boolean
  taskId: string | null // null 表示新建模式
  onClose: () => void
  ...
  requirements?: SelectOption[] // 父级需求列表
  // 默认值（新建时使用）
  defaultProductId?: string
  defaultProjectId?: string
  defaultParentId?: string // 默认父级需求
  // 子项点击回调
...
export function TaskDetailModal({
  ...
  // 当产品变更时，加载对应的模块列表
  const currentProductId = (formData.productId as string) ?? task?.productId
  useEffect(() => {
    if (!open) return
  ...
  // 当项目变更时，加载对应的迭代列表
  const currentProjectId = (formData.projectId as string) ?? task?.projectId
  useEffect(() => {
    if (!open) return
  ...
  const loadTask = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const data = await taskDetailApi.getDetail(taskId)
      setTask(data)
  ...
  }, [open, taskId, loadTask, initCreateMode])
  ...
  const handleSave = async () => {
    ...
    if (isCreateMode && task) {
      dataToSave.parentId = formData.parentId !== undefined ? formData.parentId : task.parentId
      dataToSave.productId = formData.productId !== undefined ? formData.productId : task.productId
      dataToSave.projectId = formData.projectId !== undefined ? formData.projectId : task.projectId
      dataToSave.sprintId = formData.sprintId !== undefined ? formData.sprintId : task.sprintId
      dataToSave.versionId = formData.versionId !== undefined ? formData.versionId : task.versionId
      dataToSave.moduleId = formData.moduleId !== undefined ? formData.moduleId : task.moduleId
      dataToSave.priority = formData.priority !== undefined ? formData.priority : task.priority
      dataToSave.status = formData.status !== undefined ? formData.status : task.status
      dataToSave.description = formData.description !== undefined ? formData.description : task.description
      dataToSave.plannedStartDate = formData.plannedStartDate !== undefined ? formData.plannedStartDate : task.plannedStartDate
      dataToSave.plannedEndDate = formData.plannedEndDate !== undefined ? formData.plannedEndDate : task.plannedEndDate
      dataToSave.estimatedHours = formData.estimatedHours !== undefined ? formData.estimatedHours : task.estimatedHours
      
      const assignee = formData.assignee !== undefined ? formData.assignee : task.assignee
    ...
    try {
      if (isCreateMode) {
        await taskDetailApi.create(dataToSave as any)
        message.success("创建成功")
        onSuccess?.()
        onClose()
      } else if (taskId) {
        await taskDetailApi.update(taskId, dataToSave as any)
  ...
  const handleSendComment = async () => {
    ...
    try {
      const newComment = await taskDetailApi.addComment(
        taskId,
        commentText.trim()
      )
      setTask((prev) =>
  ...
  const handleSaveNewWorkItem = async () => {
    if (!newWorkItem?.title.trim() || !taskId) {
      setNewWorkItem(null)
      return
    }

    try {
      const newChild = await taskDetailApi.addChild(taskId, {
      ...
      message.error(error.message || "创建工作项失败")
    }
  }
  ...
  // 当前显示的值
  const currentPriority = (formData.priority as string) || task?.priority || "P2"
  const currentStatus = (formData.status as string) || task?.status || "NOT_STARTED"
  ...
  const showLoading = loading || !task
          ...
          >
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0, marginRight: 16, display: "flex", alignItems: "center", gap: 12 }}>
                <ThunderboltFilled style={{ fontSize: 20, color: "#faad14", flexShrink: 0 }} />
                {editingTitle ? (
                  <Input
                    ref={titleInputRef}
                    value={titleValue}
                  ...
                  >
                    <Title
                      ...
                      style={{
                        margin: 0,
                        color:
                          formData.title || titleValue || task.title
                            ? "#1e293b"
                            : "#bfbfbf",
                      }}
                    >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                  ...
                  <Tooltip title="预估工时">
                    ...
                    >
                      ...
                      <InputNumber
                        value={(formData.estimatedHours as number) ?? task.estimatedHours}
                  ...
                  <Tooltip title="计划时间">
                    ...
                    >
                      ...
                      <DatePicker.RangePicker
                        value={[
                          (formData.plannedStartDate as string) ?? task.plannedStartDate
                            ? dayjs((formData.plannedStartDate as string) ?? task.plannedStartDate)
                            : null,
                          (formData.plannedEndDate as string) ?? task.plannedEndDate
                            ? dayjs((formData.plannedEndDate as string) ?? task.plannedEndDate)
                            : null,
                        ]}
                        onChange={(dates) => {
                  ...
                  items={[
                    {
                      ...
                      children: (
                        <div style={{ padding: 20, height: "100%", overflow: "auto" }}>
                          <TextArea
                            value={(formData.description as string) ?? task.description ?? ""}
                            onChange={(e) => updateField("description", e.target.value)}
                            placeholder="点击此处添加任务描述...&#10;&#10;可以包含：&#10;• 任务背景&#10;• 实现方案&#10;• 验收标准&#10;• 其他备注"
                            autoSize={{ minRows: 10 }}
                            style={{
                              background: "#fafafa",
                              borderRadius: 8,
                    ...
                    },
                    {
                      ...
                      label: (
                        <span>
                          <ApartmentOutlined /> 工作项 ({task.childCount})
                        </span>
                      ),
                      children: (
                        <div style={{ padding: 20, height: "100%", overflow: "auto" }}>
                          ...
                          >
                            <Space>
                              <Text type="secondary">
                                已完成 {task.completedChildCount} / {task.childCount}
                              </Text>
                              <Progress
                            ...
                            {newWorkItem?.isEditing && (
                              ...
                              >
                                ...
                                <Input
                                  ...
                                  placeholder="输入工作项标题，回车保存，ESC取消"
                                  variant="borderless"
                                  style={{ flex: 1 }}
                                />
                                <Space size={8}>
                            ...
                            {task.children.length > 0 ? (
                              task.children.map((item, index) => {
                                ...
                                return (
                                  ...
                                  >
                                    {/* 工作项图标 */}
                                    <CodeFilled style={{ color: "#60a5fa", fontSize: 14, marginRight: 10, flexShrink: 0 }} />
                    ...
                    },
                    {
                      ...
                      label: (
                        <span>
                          <CommentOutlined /> 评论 ({task.comments.length})
                        </span>
                      ),
                      children: (
                        <div style={{ padding: 20, height: "100%", overflow: "auto" }}>
                    ...
                    ...(!isCreateMode ? [{
                      ...
                      children: (
                        <div style={{ padding: 20, height: "100%", overflow: "auto" }}>
                          {task.activityLogs.filter(
                            (log) => log.action === "指派负责人"
                          ).length > 0 ? (
                            <Timeline
                              items={task.activityLogs
                                  ...
                                  children: (
                                    <div>
                                      <Space>
                                        ...
                                        <Text>变更为</Text>
                                        <Text type="success">{log.newValue || "未分配"}</Text>
                                      </Space>
                    ...
                    ...(!isCreateMode ? [{
                      ...
                      children: (
                        <div style={{ padding: 20, height: "100%", overflow: "auto" }}>
                          <Timeline
                            items={task.activityLogs.map((log) => ({
                              color: "gray",
                              children: (
                                <div>
                                  <Space wrap>
              ...
              <div style={{ flex: 1, overflow: "auto", padding: 16 }}>
                  ...
                  <div style={{ marginTop: 8 }}>
                    {task.parentTitle ? (
                      ...
                      <Select
                        size="small"
                        placeholder="选择需求"
                        value={(formData.parentId as string) ?? task.parentId}
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <Select
                    size="small"
                    placeholder="选择产品"
                    value={(formData.productId as string) ?? task.productId}
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <Select
                    size="small"
                    placeholder="选择项目"
                    value={(formData.projectId as string) ?? task.projectId}
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <Select
                    ...
                    value={(formData.sprintId as string) ?? task.sprintId}
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <Text>{dayjs(task.updatedAt).format("YYYY-MM-DD HH:mm")}</Text>
                  </div>
                </div>
              </div>
            </div>
